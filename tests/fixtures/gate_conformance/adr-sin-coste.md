...
1. **Objeto plano ({} o Object.create(null))**  
   - Ventaja: también nativo.  
   - Descartada porque: no conserva orden de inserción, el rendimiento de `Map` para claves dinámicas es superior, y `Map` expone un API más limpia (`has`, `get`, `set`, `delete`).  
2. **Array asociativo indexado por código**  
   - Descartada porque: la búsqueda por clave sería O(n) y no escalaría ni siquiera en memoria.
